...
def read(pdf_path: str, reader: str, shelf_ids: tuple[str, ...], output_dir: str) -> None:
    ...
    library.update_index(paper_id, output_dir, shelves=list(shelf_ids) if shelf_ids else None)
...
def list_papers(fmt: str, sort_by: str, shelf_id: str | None, output_dir: str) -> None:
...
def shelf_create(name: str, name_ja: str, output_dir: str) -> None:
    """Create a new shelf."""
    try:
        s = library.create_shelf(name, output_dir, name_ja=name_ja)
        console.print(f"[green]Created shelf:[/green] {s['shelf_id']} ({s['name']})")
...
def shelf_rename(shelf_id: str, new_name: str, name_ja: str | None, output_dir: str) -> None:
    """Rename a shelf."""
    try:
        s = library.rename_shelf(shelf_id, new_name, output_dir, name_ja=name_ja)
        console.print(f"[green]Renamed to:[/green] {s['shelf_id']} ({s['name']})")
    except StorageError as e:
        console.print(f"[red]Error:[/red] {e}")
        raise SystemExit(1)
...
def shelf_delete(shelf_id: str, output_dir: str) -> None:
    """Delete a shelf. Papers in the shelf will become unsorted."""
    try:
...
@click.option("--output-dir", type=click.Path(), default="library", help="Library directory")
def shelf_assign(paper_id: str, shelf_ids: tuple[str, ...], output_dir: str) -> None:
    """Assign a paper to one or more shelves (replaces existing)."""
    try:
        library.assign_paper_to_shelves(paper_id, list(shelf_ids), output_dir)
        console.print(f"[green]Assigned {paper_id} to:[/green] {', '.join(shelf_ids)}")
    except StorageError as e:
        console.print(f"[red]Error:[/red] {e}")
        raise SystemExit(1)
